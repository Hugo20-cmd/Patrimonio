...
export default function SettingsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.error) {
      ...
      setSuccessMsg("Perfil e preferí­Âªncias atualizados com sucesso!");
    ...
    // Limpa a préí¢ÂÂvisualização, pois já temos a URL definitiva
    setPreviewUrl("");
    setLoading(false);
  };
  ...
  return (
    ...
    >
      <div>
        <h1 style={{ fontSize: "1.8rem", marginBottom: "4px" }}>Configuraçí­Âµes do Perfil</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px", background: "var(--bg-elevated)", border: "1px solid var(--border-default)", borderRadius: "10px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <div>
                <h3 style={{ fontSize: "0.95rem", fontWeight: 600, margin: 0 }}>Notí­Â­cias e Alertas de Mercado</h3>
                <p style={{ fontSize: "0.85rem", color: "var(--text-tertiary)", margin: "4px 0 0 0" }}>Receba informaçí­Âµes sobre investimentos, açí­Âµes e ETFs.</p>
              </div>
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "16px" }}>
            ...
            >
              <Save size={16} />
              {loading ? "Salvando..." : "Salvar Alteraçí­Âµes"}
            </button>
          </div>
        </form>
      </div>
      ...
      <div style={{
        ...
        <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", marginBottom: "16px" }}>
          Se vocí­Âª realizou compras antes do sistema de ní­Â­veis ser implementado e não recebeu seu XP, clique no botão abaixo para o sistema recalcular todo o seu histí­Â³rico.
        </p>
        <button
          onClick={async () => {
            try {
