...
export default function AchievementsClient({ profile, unlockedKeys, userAchievements }: { profile: any, unlockedKeys: string[], userAchievements: any[] }) {
  const levelProgress = profile && profile.xpToNextLevel ? (profile.xp / profile.xpToNextLevel) * 100 : 0;
  ...
  // Merge static UI data with dynamic unlocks
  const allAchievements = mockAchievements.map(ach => {
    const isUnlocked = unlockedKeys.includes(ach.key);
    const dbData = userAchievements.find((ua: any) => ua.achievement_key === ach.key);
    return {
      ...ach,
      isUnlocked,
      unlockedAt: dbData ? dbData.unlocked_at : null
    };
  });

  // Também precisamos mostrar conquistas diní­Â¢micas que não estão no mockAchievements (ex: metas)
  const dynamicAchievements = userAchievements
    ...
    .map((ua: any) => {
      ...
      let desc = "Vocí­Âª desbloqueou algo especial.";
      let icon = "í°ÂÂÂ¯";
      if (ua.achievement_key.startsWith('meta_50pct')) {
        title = "Metade do Caminho"; desc = "Vocí­Âª atingiu 50% de uma meta!";
      } else if (ua.achievement_key.startsWith('meta_100pct')) {
        title = "Meta Concluí­Â­da!"; desc = "Vocí­Âª completou 100% da sua meta financeira!"; icon = "í°ÂÂÂ";
      } else if (ua.achievement_key === 'primeira_transacao') {
        title = "Primeiros Passos"; desc = "Vocí­Âª registrou sua primeira transação!"; icon = "í°ÂÂÂ";
      }
  ...
  const finalAchievements = [...dynamicAchievements, ...allAchievements];
      ...
      <div style={{
        ...
        <div style={{ flex: 1, position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "12px" }}>
            <div>
              <h2 style={{ fontSize: "1.4rem", marginBottom: "4px" }}>{profile?.level >= 5 ? "Investidor Pleno" : "Investidor Iniciante"}</h2>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                <Flame size={14} color="var(--orange-primary)" fill="var(--orange-primary)" />
                {profile?.level >= 5 ? "Jornada a todo vapor" : "Começando sua jornada"}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: "1.2rem", fontWeight: 800, color: "var(--blue-primary)" }}>{profile?.xp || 0} XP</div>
              <div style={{ fontSize: "0.8rem", color: "var(--text-tertiary)" }}>Faltam {(profile?.xpToNextLevel || 1000) - (profile?.xp || 0)} XP para Ní­Â­vel {(profile?.level || 1) + 1}</div>
            </div>
          </div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
          {finalAchievements.map((ach) => {
            ...
            const color = rarityColors[ach.rarity as keyof typeof rarityColors] || rarityColors.comum;
            const glow = rarityGlows[ach.rarity as keyof typeof rarityGlows] || rarityGlows.comum;
