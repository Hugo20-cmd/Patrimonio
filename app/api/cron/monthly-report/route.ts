...
export async function GET(req: Request) {
  try {
    ...
    const { data: profiles } = await supabaseAdmin
      ...
      .eq('plan', 'Premium'); // Apenas envia para Premium como benefí­cio extra
    ...
    for (const profile of profiles) {
      ...
      const htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9f9f9; padding: 20px; border-radius: 8px;">
          <h2 style="color: #333; text-align: center;">Seu Relatí³rio Mensal - Patrimônio+</h2>
          <p>Olá, <strong>${profile.name}</strong>!</p>
          <p>Aqui está o resumo do fechamento da sua carteira no íºltimo míªs:</p>
          ...
          <p>Para baixar seu relatí³rio detalhado em PDF, acesse o painel e clique em "Gerar Relatí³rio PDF".</p>
          ...
          <p style="font-size: 12px; color: #999; text-align: center; margin-top: 40px;">
            Enviado automaticamente pelo Patrimônio+. Para cancelar o recebimento, altere suas configuraçíµes de notificação.
          </p>
        </div>
      `;
      ...
      if (process.env.RESEND_API_KEY) {
        try {
          await resend.emails.send({
            ...
            to: profile.email,
            subject: 'Seu Relatí³rio Mensal de Investimentos chegou!',
            html: htmlContent,
          });
