  ...
  async function handleConnect() {
    ...
    try {
      ...
      if (data.accessToken) {
        ...
        setStatusMsg({ type: "error", text: "Não foi possí­vel gerar o token de conexão." });
      }
    } catch {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
            ...
            <h2 style={{ fontSize: "1.1rem", fontWeight: 700 }}>Open Finance â Conectar Corretora</h2>
          </div>
          <p style={{ fontSize: "0.82rem", color: "var(--text-tertiary)", maxWidth: "480px" }}>
            Conecte sua corretora via <strong style={{ color: "var(--text-secondary)" }}>Pluggy</strong> e seus ativos serão importados automaticamente â sem digitar nada.
          </p>
        </div>
        <button
      ...
      {connectedItems.length === 0 && !syncing && (
        <div style={{ padding: "24px", background: "var(--bg-elevated)", border: "1px solid var(--border-subtle)", borderRadius: "12px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: "0.78rem", color: "var(--text-tertiary)" }}>
            Compatí­vel com XP, Clear, Rico, BTG, Nubank, Inter e mais de 30 instituiçíµes
          </div>
        </div>
      )}
